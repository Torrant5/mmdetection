...
def reduce_to_two_classes_person_ball(data: dict) -> dict:
    ...
    # NOTE: We preserve the original COCO IDs to leverage pre-trained model knowledge
    keep = {1, 33}
    out = copy.deepcopy(data)
    # Keep original COCO IDs - no remapping needed
    # Use 'sports ball' (with space) to match CocoDataset expectations
    classes = [
    ...
    for ann in out.get('annotations', []):
        ...
        if cid in keep:
            # Keep the original category_id without remapping
